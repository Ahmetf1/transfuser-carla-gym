...
class RlScenarioManager(object):
    """
    ...
    def get_ego_action(self, action):
        ...
        control.steer = float(((action % 11) - 5) / 5)
        if ((action % 6) == 0):
            control.throttle = float(0)
            control.brake = float(1)
        else:
            control.throttle = float(((action % 6) - 1) / 4)
            control.brake = float(0)
        return control

    def _tick_scenario(self, timestamp, action):
        """
        ...
        if self._timestamp_last_run < timestamp.elapsed_seconds and self._running:
            ...
            try:
                self._agent()
                ego_action = self.get_ego_action(action)
