...
class PidEnv(gym.Env):
    ...
    """
        steer = ((action % 11) - 5) / 5
        actual_output = self.pid_algorithm(self.err)
        reward = 1 - ((steer-actual_output)**2)
    ...
    def render(self, mode='human', close=False):
        """
    This method provides the option to render the environment's behavior to a window
    ...
    def observe(self):
        self.err = np.random.rand(1)
        x = np.zeros((1,))
        x[0] = self.err
        return x

    def pid_algorithm(self, err):
        ...
        if len(self._window) >= 2:
            ...
            integral = 0.0
            derivative = 0.0
        pid = np.clip(self.KP * err + self.KI * integral + self.KD * derivative, -1, 1)
        return pid
...
def get_args():
    ...
    parser.add_argument('--task', type=str, default='PidEnv')
