...
class HybridAgent(autonomous_agent.AutonomousAgent):
    def setup(self, path_to_conf_file, route_index=None):
        ...
        for file in os.listdir(path_to_conf_file):
            if file.endswith(".pth"):
                ...
                state_dict = {k[7:]: v for k, v in
                              state_dict.items()}  # Removes the .module coming from the Distributed Training. Remove this if you want to evaluate a model trained without DDP.
    ...
    def run_step(self, input_data, timestamp):
        ...
        # self.nets[0].rl_controller.get_action(self.nets[0].rl_data_logger.return_data(), False)
    ...
    def prepare_image(self, tick_data):
        ...
        for degree in self.aug_degrees:
            ...
            rgb = torch.from_numpy(
                self.shift_x_scale_crop(image, scale=self.config.scale, crop=self.config.img_resolution,
